...
class WorkerAlarmCallScreen extends StatefulWidget {
  const WorkerAlarmCallScreen({super.key});

  @override
  State<WorkerAlarmCallScreen> createState() => _WorkerAlarmCallScreenState();
}

class _WorkerAlarmCallScreenState extends State<WorkerAlarmCallScreen> {
  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<WorkerViewModel>().loadAlarmHistory();
    });
  }
    ...
    showModalBottomSheet<void>(
      ...
      builder: (sheetCtx) => WorkerBottomDualAction(
        ...
        onRightTap: () {
          ...
          context.read<WorkerViewModel>().raiseAlarm();
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Alarm raised – supervisors notified'),
                    ...
                    WorkerPanelCard(
                      child: Column(
                        children: [
                          ...
                          SizedBox(height: 6.h),
                          Text(
                            'Use this only in an emergency situation.\nSupervisors and admin will be notified immediately.',
