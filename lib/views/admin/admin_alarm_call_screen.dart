...
class AdminAlarmCallScreen extends StatefulWidget {
  const AdminAlarmCallScreen({super.key});

  @override
  State<AdminAlarmCallScreen> createState() => _AdminAlarmCallScreenState();
}

class _AdminAlarmCallScreenState extends State<AdminAlarmCallScreen> {
  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<AdminViewModel>().loadAlarmHistory();
    });
  }
    ...
    showModalBottomSheet<void>(
      ...
      builder: (sheetCtx) => WorkerBottomDualAction(
        ...
        onRightTap: () {
          ...
          context.read<AdminViewModel>().raiseAlarm();
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Alarm raised – supervisors & operators notified'),
                  ...
                  children: [
                    WorkerPanelCard(
                      child: Column(
                        children: [
                          ...
                          SizedBox(height: 6.h),
                          Text(
                            'Use this only in an emergency situation.\nSupervisors and operators will be notified immediately.',
                    ...
                    if (history.isEmpty)
                      const WorkerStatusBanner(
                        ...
                        subtitle: 'No alarms have been raised yet.',
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  (entry['user_name']?.isNotEmpty == true
                          ? entry['user_name']
                          : entry['type'] ?? 'Alarm')
                      .toString(),
                  style: AppTypography.body().copyWith(
